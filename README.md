...
Rust は〈スタック〉割当てより〈ヒープ〉割当てを好みます。上の `x` はスタック上に直接置かれます。しかし、`Vec<T>` 型はベクトルの要素が置かれる領域をヒープ内に割当てます。この違いがよく分からない人は、今は気にしなくても構いませんし、[「〈スタック〉と〈ヒープ〉」][heap]を読むのも良いでしょう.
