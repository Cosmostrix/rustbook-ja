...
これはどのように動作するのでしょうか？　算譜は1から100の間の適当に選んだ整数を生成します。次に、推測を入力するようプレーヤーに促します。
推測が入力されると、推測値が低すぎるか高すぎるかを示す算譜が表示されます。
推測が正しい場合、ゲームは祝福メッセージを印字して終了します。

## 新しい企画を設定する

新しい企画を設定するには、第1章で作成した*projects*階層に移動し、Cargoを使用して新しい企画を作成します。次のようにします。
...
`--bin`フラグはCargoに第1章のような二進譜企画を作るよう指示します。第2の命令は新しい企画の階層に変わります。
...
推測ゲーム算譜の最初の部分は、利用者の入力を求め、その入力を処理し、入力が予想される形式であることを確認します。
まず、プレイヤーに推測を入力させます。
...
<span class="caption">譜面リスト2-1。利用者からの推測を​​取得して印字する譜面</span>

この譜面には多くの情報が含まれていますので、行ごとに説明しましょう。
利用者の入力を取得し、その結果を出力として出力するには、`io`（入出力）譜集を有効範囲に入れる必要があります。
...
`std::io`譜集を使用すると、利用者の入力を受け入れる機能を含む多くの便利な機能が提供されます。
...
この譜面は、ゲームが何であるかを示すプロンプトを表示し、利用者からの入力を要求しています。
...
> > 注。 `//`構文は、行末まで続く注釈を開始します。
> > Rustは注釈のすべてを無視します。これについては第3章で詳しく説明します。
...
拡張可能なUTF-8符号化されたテキスト・ビットである標準譜集ーによって提供される文字列・型です。
...
譜面の次の部分、`.read_line(&mut guess)`は、[`read_line`][read_line]
 操作法を標準入力手綱に追加して、利用者からの入力を取得します。
`read_line` 1つの引数を`read_line`ます。 `&mut guess`。

[read_line]: ../../std/io/struct.Stdin.html#method.read_line

`read_line`の仕事は、利用者が標準入力に入力したものをそのまま文字列に入れて、その文字列を引数として取り出すことです。
文字列引数は変更可能である必要があり、操作法は利用者入力を追加して文字列の内容を変更できます。
...
これまで説明してきたのは1行のテキストですが、これは単一の論理的な譜面行の最初の部分にすぎません。
2番目の部分はこの操作法です。
...
先に述べたように、`read_line`は、`read_line`文字列に利用者が入力したものを入れますが、値も返します。この場合、[`io::Result`][ioresult]
 。
...
`Result`型は[*enumerations*][enums]
 しばしば*列挙型*と呼ばれます。
列挙型は、固定値のセットを持つことができる型であり、その値は列挙型の*場合値*と呼ばれ*ます*。
...
`read_line`操作法が`Err`返す場合は、基になる基本算系からの誤りの結果である可能性があります。
...
この行は、利用者の入力を保存した文字列を出力します。中かっこ`{}`は、場所取りです。`{}`は、値を保持する小さなカニのピンチャーであると考えてください。
...
Crates.ioは、Rust生態系の人々が他の人が使用するための原譜公開のRust企画を投稿する場所です。
...
カーゴは、譜面について何も変更していないことも知っているので、それを再製譜しません。
何もすることなく、それは単に終了します。
...
Cargoには、あなたや他の誰かが譜面を組み上げするたびに、同じアーティファクトを再構築できる仕組みがあります。指定するまで、Cargoはあなたが指定した依存関係の版のみを使用します。
たとえば、`rand` crateの次の版0.3.15が出てきて、重要なバグ修正が含まれているだけでなく、譜面を壊す退行現象が含まれているとどうなりますか？　

この問題への答えは、*Cargo.lock*ファイルです。これは、最初に`cargo build`を実行したときに作成され、今*guessing_game*階層にあります。
...
通い箱を更新し*ない*場合、カーゴは別の命令、提供`update` *Cargo.lockファイル*を無視し*Cargo.toml*で仕様に合わせて、すべての最新版を把握します。
...
次に`cargo build`を実行`cargo build`、Cargoは利用可能な通い箱の登録を更新し、指定した新しい版に従って`rand`要件を再評価します。
...
今、追加したことを`rand` *Cargo.toml*に通い箱を、の使用を開始しましょう`rand`。
...
また、中央に2行追加しています。
`rand::thread_rng`機能は、使用する特定の乱数生成器を提供します。現在の実行走脈の局所的であり、基本算系によってシードされるものです。
...
[`match`][match]を使用する
 式を使用して、`guess`と`secret_number`値を持つ`cmp`の呼び出しから返された`Ordering`場合値に基づいて、次に何をするかを決定します。
...
`match`コンストラクトとパターンはRustの強力な機能で、譜面で発生する可能性のあるさまざまな状況を表現し、それらをすべて処理することを保証します。
...
利用者が<span class="keystroke">enterを</span>押すと、改行文字が文字列に追加されます。
...
第3章で他の数値型についても学習します。さらに、このサンプル算譜の`u32`補注と`secret_number`との比較は、Rustが`secret_number`も`u32`なければならないことを意味します。
今比較は同じ型の2つの値の間になります！　
...
ナイス！　
推測の前に空白が追加されたにもかかわらず、算譜は利用者が76を推測したと考えました。異なる種類の入力で異なる動作を確認するために算譜を数回実行します。数字を正しく推測し、あまりにも低い数字を推測してください。
...
利用者はキーボード近道<span class="keystroke">ctrl-c</span>を使用して算譜を停止でき<span class="keystroke">ます</span>。
しかし、「推測を秘密の数字に比較する」の`parse`ディスカッションで述べたように、この不安定なモンスターから脱出する別の方法があります。利用者が非数字の答えを入力すると、算譜が異常終了します。
...
利用者が`break`文を追加して勝利したときにゲームを終了するように算譜しましょう。
...
`You win!`後に`break`行を追加すると、利用者が秘密の番号を正しく推測したときに算譜がループを終了します。
ループを終了すると、ループが`main`最後の部分であるため、算譜を終了することも意味します。

### 無効な入力の処理

利用者が非数字を入力したときに算譜を異常終了させるのではなく、ゲームの動作をさらに洗練するために、ゲームが推測を続けることができるように数字以外の数字を無視してみましょう。
