...
[crates.io](https://crates.io)の通い箱レジストリ
 パッケージの原譜を配布するので、主に原譜公開の譜面をホストします。
...
第3章では、2つのスラッシュを使ってRust譜面を注釈する方法について説明しました`//`。
Rustには、HTML開発資料を生成する*開発資料集注釈*として便利な、開発資料に関する特別な種類の注釈もあります。
HTMLには、通い箱の*実装*方法とは対照的に、通い箱の*使用*方法を知りたい演譜師向けの公開API項目の開発資料注釈の内容が表示されます。

開発資料注釈では、2つの代わりに3つのスラッシュ（`///`使用し、テキストの書式設定のためのMarkdown表記をサポートしています。
文書化している項目の直前に文書の注釈を置いてください。
リスト14-1はのための開発資料集注釈を示し`add_one`通い箱という名前で機能`my_crate`。
...
`cargo doc`実行することによって、この開発資料注釈からHTML開発資料を生成することができます。
この命令は、Rustと一緒に配布されている`rustdoc`道具を実行し、生成されたHTML開発資料を*target/doc*階層に置きます。

便宜上、`cargo doc --open` -openは現在の通い箱の文書（通い箱のすべての依存物に関する文書）用のHTMLを作成し、その結果をWebブラウザで開きます。
`add_one`機能に`add_one`すると、図14-1に示すように、開発資料注釈内のテキストがどのように表示されるかがわかります。
...
ほとんどの開発資料集注釈はこれらの章のすべてを必要としませんが、譜面を呼び出す人々が知りたいと思う譜面の面を思い出させる良いチェックリストです。

#### テストとしての開発資料集注釈

開発資料集注釈に譜面例ブ​​ロックを追加すると、譜集の使用方法を示すのに役立ちます。そうすることで、追加のボーナスが得られます。実行中の`cargo test`は、開発資料集の譜面例をテストとして実行します。
...
開発資料注釈の別の作法`//!`は、注釈に続く項目に開発資料を追加するのではなく、注釈を含む項目に開発資料を追加します。
通常、*crate*ルートファイル（慣習的に*src/lib.rs*） *内また*はこれらの開発資料の注釈を役区全体で使用して、通い箱または役区全体を文書化します。

たとえば、`add_one`機能を含む`my_crate`の目的を記述した開発資料を追加したい場合は、*src/lib.rs*ファイルの先頭に`//!`で始まる開発資料注釈を追加することができます14-2。
...
`///`ではなく`//!`を使用して注釈を開始したので、この注釈の後に続く項目ではなく、この注釈を含む項目を文書化しています。
この場合、この注釈を含む項目は*src/lib.rs*ファイルであり、これは通い箱ルートです。
これらの注釈は通い箱全体を記述します。

`cargo doc --open`を実行すると、これらの注釈は、図14-2に示すように、通い箱内の公開項目のリストの上にある`my_crate`の開発資料のフロントページに表示されます。

<img src="img/trpl14-02.png" alt="通い箱の注釈全体をHTML形式で表示" class="center" />
<span class="caption">図14-2。 <code>my_crate</code>描出された開発資料。通い箱全体を説明する注釈を含む</span>

項目内の開発資料集注釈は、特に通い箱や役区の説明に役立ちます。
これらを使用して、コンテナの全体的な目的を説明し、利用者が通い箱の組織を理解するのを助けます。

### `pub use`した便利な公開APIの輸出

第7章では、`mod`予約語を使用して譜面を役区に編成する方法、`pub`予約語を使用して項目を公開にする方法、`use`予約語を`use`して項目を有効範囲に持ち込む方法について説明しました。
しかし、通い箱を開発している間にあなたに合った構造は、利用者にはあまり便利でないかもしれません。
...
公開APIの構造は、通い箱を公開する際の主な考慮事項です。
通い箱を使用する人はものより構造にあまり精通しておらず、通い箱が大きな役区階層を持っている場合、使用したい部分を見つけるのが難しいかもしれません。
...
リスト14-4に示すように、`art`通い箱利用者はリスト14-3の内部構造を見たり使用したりすることができます。リスト14-6で示すように、リスト14-5のより便利な構造を使用することもできます。
...
便利な公開API構造を作成することは、科学よりも芸術であり、利用者にとって最適なAPIを見つけることができます。
`pub use`を選択すると、通い箱を内部的にどのように構築するかに柔軟性がもたらされ、その内部構造を利用者に提示するものから切り離すことができます。
...
その理由は、いくつかの重要な情報が欠落しているということです。通い箱が何をしているのか、どのような条件で使用できるのかを人々が知るための説明とライセンスが必要です。
この誤りを修正するには、この情報を*Cargo.toml*ファイルに含める必要があります。

通い箱と一緒に検索結果に表示されるため、1つまたは2つの説明だけを追加します。
`license`欄では、*ライセンスIDの値を指定*する必要があり*ます*。
[Linux Foundationの譜体パッケージデータ交換（SPDX）に][spdx]は、この値に使用できる識別子がリストされています。
たとえば、MITライセンスを使用して通い箱をライセンスしたことを指定するには、`MIT`識別子を追加します。
...
[Cargoの開発資料に](https://doc.rust-lang.org/cargo/)は、他の人が通い箱をより簡単に発見して使用できるように指定できる他のメタデータが記載されています。

### Crates.ioへの公開

アカウントを作成し、API字句を保存し、通い箱の名前を選択し、必要なメタデータを指定したら、すぐに公開できます！　
...
おめでとう！　
Rustコミュニティと譜面を共有しました。誰でも企画の依存として通い箱を簡単に追加できます。

### 既存の通い箱の新しい版を公開する

通い箱を変更して新しい版をリリースする準備ができたら、*Cargo.toml*ファイルで指定された`version`値を変更して再発行します。
