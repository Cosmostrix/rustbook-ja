...
この例では、譜面で式したいと思う状況を見て、列挙型が構造体よりも有用で適切な理由を見てみましょう。
IP番地で作業する必要があるとします。
...
列挙型の値は変種の1つに過ぎないため、IP番地のそのプロパティは列挙型のデータ構造を適切にします。
版4と版6の両方の番地は、基本的にはIP番地なので、譜面があらゆる種類のIP番地に適用される状況を処理する場合、同じ型として扱う必要があります。

`IpAddrKind`列挙を定義し、IP番地が`V4`と`V6`の可能な種類を列挙することで、この概念を譜面で式できます。
これらは列挙*型の変種*として知られてい*ます*。
...
`IpAddrKind`ように、`IpAddrKind`の2つの変種それぞれの実例を作成できます。
...
列挙型の変種はその識別子の下に名前空間があり、二重のコロンを使用してそれらを区切ることに注意してください。
...
そして、この機能をどちらかのバリエーションで呼び出すことができます。
...
構造体内の列挙型ではなく、単に列挙型を使用して、各列挙型場合値に直接データを入れて、同じ概念をより簡潔な方法で式できます。
...
版4種類のIP番地は、常に保存したい場合は0〜255の値を持つことになります4つの数字の部品があります`V4` 4つのとして番地を`u8`値が、それでも式`V6`一つとして、番地を`String`値を、持つことができないだろう構造体。
列挙型はこのケースを簡単に処理します。
...
しかし、明らかになったように、IP番地を格納して、どのような種類のものを符号化したいのかは、よくあることです[。標準譜集には、使用できる定義があります。][IpAddr]
 標準譜集が`IpAddr`をどのように定義しているかを見てみましょう。それは定義し使用した正確な列挙型と変種を持っていますが、変種ごとに異なって定義された2つの異なる構造体の形で、
...
前の章では、`IpAddr`列挙型がRustの型システムを使用して、算譜のデータだけでなく、より多くの情報を符号化する方法を見てきました。
この章では、標準譜集によって定義された別の列挙型である`Option`事例で学ぶについて説明します。
`Option`型は、値が何かになる可能性がある、または何もできないという非常に一般的な場合を符号化するため、多くの場所で使用されます。
この概念を型システムの観点から式すると、製譜器は処理する必要があるすべてのケースを処理したかどうかをチェックできます。
...
これは総称型のパラメータであり、第10章で総称化について詳しく説明します。今のところ、`<T>`は`Option` enumの`Some` variantが任意の型のデータを保持できることを意味します。
...
言い換えれば、あなたは変換する必要があります`Option<T>`する`T`あなたが実行する前に`T`それとの動作を制御します。
...
どこでも値がない型があることを`Option<T>`、あなたは安全に値がnullでないと仮定*することができ*ます。
...
`match`式は、列挙型で使用されている場合にのみこれを行う制御の流れ構造です。列挙型の変種に応じて異なる譜面を実行し、一致する値内のデータを使用することができます。
