...
たとえば、リスト5-1は、ユーザーアカウントに関する情報を格納する構造体を示しています。
...
このユーザーの電子メール番地だけが必要`user1.email`場合は、この値を使用したい場所で`user1.email`を使用できます。
...
リスト5-4に、指定した電子メールとユーザー名で`User`実例を返す`build_user`機能を示します。
...
<span class="caption">リスト5-4。電子メールとユーザー名をとり、 <code>User</code>実例を返す<code>build_user</code>機能</span>
