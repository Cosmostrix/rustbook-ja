...
Rustは、基本算系と直接対話する、または独自の基本算系を作成するなど、低水準の算系演譜を可能にする必要があります。
低水準の算系演譜を使用することは、言語の目標の1つです。
...
時々、Rustの譜面は、別の言語で書かれた譜面と対話する必要があるかもしれません。
...
`"C"`部分は、外部機能が使用する*譜体二進譜接点（ABI）を*定義します。ABIは、アセンブリ水準で機能を呼び出す方法を定義します。
...
> > また、`#[no_mangle]`補注を追加して、この機能の名前をmangleしないようにRust製譜器に指示する必要があります。
