...
今、提供機は各要求を順番に処理します。つまり、最初の処理が完了するまで2番目の接続を処理しません。
提供機がますます多くのリクエストを受信した場合、このシリアル実行は、それほど最適ではありません。
提供機が処理に時間がかかる要求を受信した場合、新しい要求を迅速に処理できる場合でも、後続の要求は長い要求が完了するまで待機する必要があります。
これを修正する必要がありますが、最初に実際の問題を見ていきます。

### 現在の提供機実装で低速要求をシミュレートする

処理の遅いリクエストが、現在の提供機実装に対する他のリクエストにどのように影響するかを見ていきます。
リスト20-10は、*/* sleptへのリクエストの処理をシミュレートした低速レスポンスで実装しています。これにより、提供機は応答する前に5秒間スリープします。
...
提供機が認識するデータを持つ2番目のリクエスト`sleep`を作成しました。
`if`段落の後に`else if`を追加して、*/ sleep*へのリクエストを確認しました。
その要求を受け取ると、提供機は5秒間スリープしてからHTMLページが正常に表示されます。

提供機はどれほど基本型なのかを見ることができます。実際の譜集は、あまり冗長ではない複数のリクエストの認識を処理します！　

使用して提供機を起動し`cargo run`。
...
遅い要求の後ろにもっと多くの要求が戻ってくることを避けるためにWeb提供機の仕組みを変更する方法はいくつかあります。
実装するのは走脈貯留庫です。
...
走脈貯留庫を使用すると、接続を同時に処理できるため、提供機のスループットが向上します。

サービス拒否（DoS）攻撃から私たちを守るために、貯留庫内の走脈の数を少数に制限します。
算譜が入ってくるごとにリクエストごとに新しい走脈を作成した場合、提供機に1000万回のリクエストを行う人は、提供機のすべての資源を使い切り、要求の処理を中断することによって混乱を招く可能性があります。
...
お使いのブラウザでこの譜面と読み込み*/スリープを*実行すると*、/* 2以上のブラウザのタブで、確かに*/*へのリクエストが終了するのを*/スリープ*待つ必要がないことがわかります。
しかし、言及したように、これは最終的に算系を圧倒するでしょう。なぜなら何の制限もなく新しい走脈を作るからです。
...
`ThreadPool`実装は、Web提供機がやっている仕事の種類から独立しています。
...
次に、新しい階層*src/binを*作成し、*src/main.rs*をルートとする二進譜・通い箱を*src/bin/main.rsに移動し*ます。
そうすることで、譜集は*hello*階層の一次通い箱になります。
...
`ThreadPool`開発資料集注釈付きの開発資料をいくつか追加しました。
...
野心的だと感じたら、両方の版を比較するために、次の型指示で`new`版を作成してみてください。
...
走脈の作成に関して、リスト20-14の`for`ループに注釈を残しました。
...
外部譜面（*src/bin/main.rsの*提供機のような）は、`ThreadPool`内の`Worker`構造体の使用に関する実装の詳細を知る必要はないので、`Worker`構造体とその`new`機能をprivateにします。
...
決して4つ以上の走脈が作成されることはありません。そのため、提供機が多くの要求を受け取ると、算系が多重定義になることはありません。
*/ sleepを*要求すると、提供機は別の走脈に別の走脈を実行させることで、他の要求を提供することができます。
