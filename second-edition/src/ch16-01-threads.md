...
ほとんどの現在のオペレーティングシステムでは、実行された算譜の譜面は*過程*内で実行され、オペレーティングシステムは複数の過程を一度に管理します。
...
Rustは走脈の使用による悪影響を緩和しようとしますが、マルチ走脈文脈での演譜は注意深く考えており、単一走脈で実行されている算譜とは異なる譜面構造を必要とします。

演譜言語は、いくつかの異なる方法で走脈を実装します。
多くのオペレーティングシステムでは、新しい走脈を作成するためのAPIが提供されています。
言語がオペレーティングシステムAPIを呼び出して走脈を作成するこの模型は、時には*1。1*と呼ばれ、1つの言語走脈あたり1つのオペレーティングシステム走脈を意味します。

多くの演譜言語は、独自の走脈の実装を提供しています。
演譜言語提供走脈は*緑色*走脈と呼ばれ、これらの緑色走脈を使用する言語は異なる数のオペレーティングシステム走脈の文脈で実行されます。
このため、緑色走脈模型は*M。N*模型と呼ばれ*ます。N* `N`オペレーティングシステム走脈ごとに`M`緑色走脈があります`M`と`N`は必ずしも同じ数ではありません。
...
Rustはそのような低レベル言語なので、走脈の実行時間を制御し、文脈切り替えのコストを削減するなどの面でオーバーヘッドを交換する場合は、M。N走脈を実装するひな型があります。
...
譜面リスト16-1の例は、メイン走脈の文言と新しい走脈の他の文言を出力します。
...
走脈はおそらく順番に実行されますが、それは保証されません。オペレーティングシステムが走脈をスケジュールする方法によって異なります。
この実行では、生成された走脈のprint文が譜面の最初に表示されていても、メイン走脈が最初に印字されます。
そして、`i`が9になるまで、生成された走脈に印字するように指示したとしても、メイン走脈がシャットダウンする前に5にしか達しませんでした。

この譜面を実行してメイン走脈の出力のみを表示するか、重複が見られない場合は、範囲内の数値を増やして、オペレーティングシステムが走脈間で切り替える機会を増やしてみてください。
...
このテクニックは、値の所有権をある走脈から別の走脈に移すために新しい走脈を作成するときに特に便利です。
