...
Rust型のシステムには、本書で述べたいくつかの機能がありますが、まだ説明はしていません。
...
この長い型を関数型指示と型注釈として譜面全体に記述すると、面倒で誤りが発生する可能性があります。
...
この譜面は読み書きがはるかに簡単です！　
型別名の意味のある名前を選択することで、あなたの意図を伝えるのに役立ちます（*サンク*は後で評価される譜面のため、保存される閉包の適切な名前です）。
...
当時、この譜面で詳細をスキップしました。
第6章「 `match`制御の流れ演算子」の章では、`match`アームはすべて同じ型を返さなければならないと説明しました。
たとえば、次の譜面は機能しません。
...
あなたが推測したように、`continue`は`!`値を持っています。
...
特定の型の値にどれくらいの空間を割り当てるかなど、細部を知る必要があるため、混乱を招く可能性のある型システムの一角があり*ます*。 *動的なサイズの型の*概念です。
...
どうしようか？　
この場合、あなたはすでに答えを知っています。つまり、`s1`と`s2`型を`&str`ではなくa `&str`し`str`。
