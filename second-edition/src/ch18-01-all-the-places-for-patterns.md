...
この例では、実際の算譜が利用者入力から受け取る可能性のあるハード譜面された値を持つ変数を作成しました。
...
今日が火曜日の場合、背景色は緑色です。
利用者が年齢を文字列として指定し、数値として正常に解析できる場合、数値は数値の値によって紫色またはオレンジ色になります。
...
最後の`else`段落を省略していくつかのケースを処理しなかった場合、製譜器は論理バグを警告しませんでした。
...
<span class="caption">リスト18-6。機能型指示は、パラメータのパターンを使用します。</span>
