...
この問題は第2の問題にも関連してい`query`と`filename`は算譜の構成変数ですが、`f`や`contents`などの変数を使用して算譜のロジックを実行します。
...
今の状況では、`file not found`誤りメッセージが表示され、ユーザーに間違った情報が与えられます。

第4に、さまざまな誤りを繰り返し処理する`expect`ています。ユーザーが十分な引数を指定せずに算譜を実行すると、問題を明確に説明していないRustから`index out of bounds`誤りが発生します。
誤り処理ロジックを変更する必要がある場合は、将来のメンテナーが譜面内で相談する場所が1つしかないため、すべての誤り処理譜面が1か所にある場合に最適です。
すべての誤り処理譜面を1か所にまとめることで、エンドユーザーにとって意味のあるメッセージを確実に印字できるようになります。
...
* *main.rs*と*lib.rs*にあなたの算譜を分割し*、lib.rs*にあなたの算譜のロジックを移動します。
* 命令行の解析ロジックが小さい限り、
*main.rs。*
*命令行の解析ロジックが複雑になると、*main.rs*から抽出して*lib.rs*に移動し*ます*。

この過程後の`main`機能に残っている責任は、次のように限定されます。

* 引数値を使用して命令行解析ロジックを呼び出す
* 他の設定を設定する
* *lib.rsで* `run`機能を*呼び出す*
* `run`誤りを処理すると誤り`run`返される

このパターンは、懸念を分離についてです*。main.rs*手綱は、算譜を実行している、と*lib.rsは*手元の作業のすべてのロジックを処理します。
`main`機能を直接テストすることはできないので、この構造体を使用すると、*lib.rs*内の機能に算譜のロジックを移動することで、すべての算譜のロジックをテストできます。
*main.rsに*残っている唯一の譜面は、*それ*を読み取って正当性を検証するのに十分小さいものです。
この過程を踏んで算譜を修正しましょう。

#### 引数構文解析器ーの抽出

命令行解析ロジックを*src / lib.rsに*移動する準備をするために`main`が呼び出す機能に引数を解析する機能を抽出します。
...
次に、`parse_config`機能は、どの引数がどの変数に入り、その値を`main`戻すかを決定するロジックを保持します。
...
> > あなたの最初のパスで譜面をハイパーオプティマイズしようとするよりも少し非効率な作業算譜を用意する方が良いでしょう。
...
これまで、`main`から命令行引数を解析するロジックを抽出し、`parse_config`機能に配置しました。
...
エンドユーザーが何が起こったのか、そして何をすべきかを理解するのに役立つものではありません。
今すぐ修正しましょう。
...
その後、変更することができ`main`変換すること`Err`について周囲の文言なしでユーザーの皆様のために、より実用的な誤りに場合値を`thread 'main'`と`RUST_BACKTRACE`呼び出しがする`panic!`の原因。
...
`new`機能の本体に2つの変更が加えられました。ユーザーが十分な引数を渡さないときに`panic!`を呼び出す代わりに、`Err`値を返し、`Config`戻り値を`Ok`包みました。
...
すばらしいです！　
この出力は、ユーザーにとって非常に面白いです。

### `main`からロジックを抽出する

コンフィグレーション解析のリファクタリングが完了したので、算譜のロジックを見てみましょう。
「二進譜企画の懸念の分離」で述べたように、設定や処理誤りの設定に関係しない`main`機能内のすべてのロジックを保持する機能`run`を抽出します。
作業が完了したら、`main`は簡潔になり、検査で簡単に確認できるようになり、他のすべてのロジックのテストを書くことができます。
...
<span class="caption">リスト12-11。残りの算譜ロジックを含む<code>run</code>機能の抽出</span>

`run`機能には、`main`からの残りのロジックがすべてファイルの読み込みから開始されるようになりました。
`run`機能は、`Config`実例を引数として取ります。

#### `run`機能からの誤りの返却

リスト12-9の`Config::new`に、残りの算譜ロジックを`run`機能に分けて誤り処理を改善することができます。
`expect`を呼び出す`expect`によって算譜がパニックに陥るのを防ぐ代わりに、何かがうまくいかないときに`run`機能は`Result<T, E>`を返します。
これにより、ユーザーフレンドリーな方法で誤りを処理するロジックを`main`ロジックにさらに統合することができます。
...
譜面リスト12-10の`Config::new`で使用したのと似たテクニックを使用して誤りをチェックして処理しますが、わずかな違いがあります。
