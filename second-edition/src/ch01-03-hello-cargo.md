...
企画。
*企画*階層（または譜面を保存することを決めた場所）に戻ます。
次に、すべての基本算系で、次の命令を実行します。
...
企画*hello_cargo*に名前をつけ、Cargoは同じ名前の階層にファイルを作成します。

*hello_cargo*階層に移動し、ファイルを一覧表示します。
Cargoは2つのファイルと1つの階層、*つまりCargo.toml*ファイルと*main.rs*ファイルのある*src*階層を生成していることが*わかり*ます。
また、*.gitignore*ファイルとともに新しいGitリポジトリを初期化しました。

> > 注意。Gitは一般的な版管理算系です。
> > `--vcs`フラグを使用して、別の版管理算系を使用するか、または版管理算系を使用しないように、`cargo new`を変更することができます。
> > `cargo new --help`を実行すると、使用可能な選択肢が表示されます。

選択したテキスト書房で*Cargo.toml*を開きます。
リスト1-2の譜面に似ているはずです。
...
ための算譜は、リスト1-1で書いたのとまったく同じです！　
これまでの企画とCargo企画の違いは、Cargoが*src*階層に譜面を配置し、*Cargo.toml*構成ファイルがトップ階層にあることです。

Cargoは原本が*src*階層内に存在することを期待しています。
最上位の企画階層は、READMEファイル、ライセンス情報、設定ファイル、および譜面に関係のないその他のものです。
Cargoを使用すると、企画を整理するのに役立ちます。
すべての場所があり、すべてがその場所にあります。

あなたがハロー、世界で行ったように、カーゴを使用しない企画を開始した場合！　
それをカーゴを使用する企画に変換することができます。
企画譜面を*src*階層に移動し、適切な*Cargo.toml*ファイルを作成します。

### カーゴ企画の構築と実行

さあ、こんにちは、世界を構築して実行するときの違いを見てみましょう！　
カーゴとの算譜！　
*hello_cargo*階層から、次の命令を入力して企画を組み上げします。
...
この命令は、現在の階層ではなく、*target/debug/hello_cargo*（またはWindowsでは*target \ debug \ hello_cargo.exe*）に実行可能ファイルを作成します。
...
譜面の作成中に作業を継続的にチェックしている場合は、`cargo check`を使用`cargo check`と過程が速度アップします！　
...
* 組み上げ結果を譜面と同じ階層に保存する代わりに、Cargoは*目標/虫取り*階層に格納します。

Cargoを使用することのもう一つの利点は、作業している基本算系に関係なく命令が同じであることです。
だから、現時点では、LinuxとmacOS対Windowsのための具体的な手順は提供しなくなります。

### リリースのための組み上げ

企画が最終的にリリース準備が整ったら、`cargo build --release`を使って最適化で製譜`cargo build --release`ことができます。
...
実際、既存の企画を操作するには、次の命令を使用してGitを使って譜面をチェックアウトし、その企画の階層に変更して組み上げします。
