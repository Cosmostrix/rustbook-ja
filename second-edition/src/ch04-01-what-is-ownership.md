...
Rustは第3のアプローチを使用しています。記憶は、製譜時に製譜器がチェックする一連の規則を持つ所有権算系によって管理されます。
所有権機能は、実行中に算譜の速度を低下させるものではありません。

所有権は多くの演譜師のための新しい概念なので、慣れるまでに時間がかかります。
良い情報は、Rustと所有権算系のルールにもっと熟練すればするほど、安全かつ効率的な譜面を自然に開発できるようになることです。
それをつけろ！　
...
> > しかし、Rustのような算系演譜言語では、値が山上にあるか原上にあるかは、言語の振る舞いや特定の決定を下さなければならない理由に大きな影響を与えます。
...
> > 基本算系は、原内のどこかに空きがあり、十分な大きさの空き領域を見つけ出し、使用中であるとマークし、その場所の番地である*指し手*を返します。
...
> > 同様のことを続けると、多くのテーブルから注文を受けているレストランの提供機を考えてみましょう。
...
変数`s`は文字列直書きを参照し、文字列の値は算譜のテキストにハード譜面されます。
変数は、宣言されているポイントから現在の*有効範囲の*終わりまで*有効*です。
リスト4-1には、変数`s`が有効な場所に注釈を付ける注釈があります。
...
文字列直書きは便利ですが、テキストを使用したいあらゆる状況には適していません。
1つの理由は、それらが不変であるということです。
もう1つは、譜面を書くときに文字列の値がすべてわかっているわけではないことです。たとえば、利用者の入力を受け取り、格納する場合はどうすればいいでしょうか？　
このような状況の場合、Rustには2番目の文字列型`String`ます。
この型は原上に割り当てられ、製譜時にはわからない量のテキストを格納することができます。
...
文字列直書きの場合、製譜時に内容がわかるので、テキストは最終実行ファイルに直接ハード譜面されます。
これが、文字列直書きが高速で効率的な理由です。
しかし、これらのプロパティは文字列直書きの不変性からのみ発生します。
残念ながら、製譜時にサイズが不明で、算譜の実行中にサイズが変更される可能性がある各テキストの二進譜には、一塊の記憶を入れることはできません。

`String`型では、変更可能な拡張可能なテキストをサポートするために、製譜時には未知の原上に内容を保持するための記憶量を割り当てる必要があります。
これの意味は。

* 実行時に基本算系から記憶を要求する必要があります。
* `String`が完了したら、この記憶を基本算系に返す方法が必要です。
...
`String`が必要とする記憶を基本算系に返すことができる自然なポイントがあります。 `s`が範囲外になるとき。
...
容量は、`String`が基本算系から受け取った和記憶量（バイト単位）です。
...
この表現は図4-3のように*は*見えませ*ん*.Rustが原データをコピーした場合の記憶域の様子です。
...
譜面リスト4-3には、変数が有効範囲のどこに出入りするかを示す補注の例があります。
...
`s`と`x`を使用する`main`譜面を追加して、どこでそれらを使用できるか、そして所有権の規則によってそうするのを妨げている場所を見てください。

### 戻り値と範囲

値を返すことで所有権を譲渡することもできます。
リスト4-4は、リスト4-3の補注と同様の補注を持つ例です。
