...
符号付き数値は、2の補数表現を使用して格納されます（これが何であるかわからない場合は、オン行で検索できますが説明は本書の範囲外です）。
...
もし確信が持てないなら、Rustの黙用が一般的に良い選択肢であり、整数型は自動的に`i32`ます。この型は、64ビット算系でさえ一般的に最も高速です。
...
どのように取り上げる`if`の式は、「制御構造」の章でRustに働きます。
...
多くの低水準言語では、このようなチェックは行われず、間違った添字を指定すると無効な記憶にアクセスすることができます。
