...
Rustの型のシステムと所有権のルールは、この管理を正しく行うのに大いに役立ちます。
...
ロックは、現在データに排他的にアクセスできるユーザーを追跡するミューテックスの一部であるデータ構造です。
...
しかし、Rustの型のシステムと所有権のおかげで、あなたはロックとロック解除を間違えることはありません。
...
型システムは、の値を使用する前にロックを取得することを保証`m`。 `Mutex<i32>`ない`i32`ので、使用できるようにロックを取得*しなければならない* `i32`値。
忘れることができません。
型システムは、それ以外の場合は内部の`i32`アクセスさせません。

あなたが疑うように、`Mutex<T>`はスマートな指し手です。
...
最初のインライン誤りは、```std::rc::Rc<std::sync::Mutex<i32>>` cannot be sent between threads safely``。
