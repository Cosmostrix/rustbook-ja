...
ルストをあなたが上に構築できる堅固な基盤にしたいと思っています。物事が絶えず変化しているのなら、それは不可能でしょう。
...
各アップグレードは無痛であるべきですが、新機能、バグの削減、製譜時間の短縮も必要です。
...
ほとんどのRustユーザーはベータ版を積極的に使用していませんが、Rustが可能な退行現象を発見するのを支援するために、CIシステムのベータ版をテストします。
...
ユーザーとして、作業中の機能を試してみることができますが、夜間リリースのRustを使用し、原譜に適切なフラグを付けてオプトインする必要があります。
...
導入されているすべての*道具チェーン*（Rustのリリースと関連部品）が`rustup`状態で`rustup`れます。
あなたの作者のWindows計算機の例を以下に示します。
...
ご覧のとおり、安定した道具チェーンが自動的にす。
ほとんどのRustユーザーは、ほとんどの時間安定して使用します。
ほとんどの場合、安定した使い方をしたいと思うかもしれませんが、夜間には特定の企画で使用することをお勧めします。
そうするためには、使用することができます`rustup override` 1として、毎晩道具チェーンを設定し、その企画のディレクトリに`rustup`あなたがそのディレクトリにいるときに使用する必要があります。
...
適切なチームが提案とコメントを読み、独自のコメントを書き、最終的には機能を受け入れるか拒否するかの合意があります。
