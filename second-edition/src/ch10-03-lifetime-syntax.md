...
あなたが見ることができるように、内側の`'b`段落`'a`外側`'a`段落`'a`よりもはるかに小さいです。
...
手引き文言は、返された参照が`x`または`y`を参照しているかどうかをRustが知ることができないため、戻り値の型に汎用の寿命パラメータが必要であることを示しています。
...
寿命注釈は、寿命に影響を与えることなく、複数の参照の寿命の相互関係を記述します。
...
この型指示で式したい制約は、パラメータ内のすべての参照と戻り値の寿命が同じでなければならないということです。
...
関数型指示`'a`、生存時間`'a`に対して、機能は2つのパラメータをとります。どちらも少なくとも寿命`'a`長さと同じ長さの文字列スライスです。
機能の型指示は、機能から返された文字列スライスが少なくとも寿命`'a`長さで存続することをRustに伝えます。
これらの制約は、Rustに要求しているものです。
この関数型指示で寿命パラメータを指定するとき、渡された値または返された値の有効期間は変更されません。
...
この機能が寿命注釈なしで製譜される理由は歴史的です。Rustの初期版（1.0より前）では、すべての参照で明示的な存続期間が必要だったため、この譜面は製譜されませんでした。
その時、関数型指示は次のように書かれています。

```rust,ignore
fn first_word<'a>(s: &'a str) -> &'a str {
```

多くのRust譜面を書いた後、Rustチームは、Rust演譜師が特定の状況で何度も同じ寿命注釈を何度も繰り返していたことを発見しました。
...
この場合、製譜器は、推測の代わりに、参照が互いにどのように関係しているかを指定する寿命注釈を追加することで解決できる誤りを表示します。

機能または操作法のパラメータの*寿命*は*入力寿命*と呼ばれ、戻り値の*寿命*は*出力寿命*と呼ばれ*ます*。

製譜器は、3つのルールを使用して、明示的な注釈がない場合に参照される寿命を把握します。
...
この関数型指示の中のすべての参照は寿命を持ち、製譜器はこの関数型指示の寿命に注釈を付けることなく演譜師が分析を続けることができます。
...
この文字列の文言は、算譜の二進譜に直接格納されます。この二進譜は、常に利用可能です。
したがって、すべての文字列直書きの有効期間は`'static`です。

誤りメッセージで`'static`寿命`'static`を使用することをおすすめします。
しかし、参照の寿命として`'static`指定する前に、実際に参照があなたの算譜の寿命にわたって生きているかどうかを考えてください。
...
第19章では、寿命の注釈を含むより複雑な場合や、いくつかの高度な型のシステム機能について説明します。
しかし、次に、Rustでテストを書く方法を学ぶので、あなたの譜面が確実に動作するようにすることができます。
