...
|`!`|`!expr`|<!--Bitwise or logical complement-->ビットまたは<ruby>論理<rt>ロジック</rt></ruby>補数|`Not`|
...
|`&`|<!--`&type`, `&mut type`, `&'a type`, `&'a mut type`-->`&type`、 `&mut type`、 `&'a type`、 `&'a mut type`|<!--Borrowed pointer type-->借用<ruby>指し手<rt>ポインタ</rt></ruby>型| |
...
|`*`|<!--`*const type`, `*mut type`-->`*const type`、 `*mut type`|<!--Raw pointer-->生<ruby>指し手<rt>ポインタ</rt></ruby>| |
...
|`->`|<!--`fn(...) -> type`, `\|...\| -> type`-->`fn(...) -> type`、 `\|...\| -> type``\|...\| -> type`|<!--Function and closure return type-->機能と<ruby>閉包<rt>クロージャー</rt></ruby>の戻り値の型| |
|`.`|`expr.ident`|<!--Member access-->要素アクセス| |
|`..`|<!--`..`, `expr..`, `..expr`, `expr..expr`-->`..`、 `expr..`、 `..expr`、 `expr..expr`|<!--Right-exclusive range literal-->右排他的<ruby>直書き<rt>リテラル</rt></ruby>| |
|`..`|`..expr`|<!--Struct literal update syntax-->構造体<ruby>直書き<rt>リテラル</rt></ruby>の更新構文| |
|`..`|<!--`<ruby>場合値<rt>バリアント</rt></ruby>(x, ..)`, `struct_type { x, .. }`-->`<ruby>場合値<rt>バリアント</rt></ruby>(x, ..)`、 `struct_type { x, .. }`|<!--“And the rest” pattern binding-->"そして残りの"パターン束縛| |
...
|`:`|`ident: expr`|<!--Struct field initializer-->構造体<ruby>欄<rt>フィールド</rt></ruby>初期化子| |
...
|`=>`|`pat => expr`|<!--Part of match arm syntax-->マッチ分岐構文の一部| |
...
|`^`|`expr ^ expr`|<!--Bitwise exclusive OR-->ビットごとの排他的<ruby>論理<rt>ロジック</rt></ruby>和|`BitXor`|
|`^=`|`var ^= expr`|<!--Bitwise exclusive OR and assignment-->ビット単位の排他的<ruby>論理<rt>ロジック</rt></ruby>和と代入|`BitXorAssign`|
|<code>\|</code>|`pat \| pat`|<!--Pattern alternatives-->パターンの<ruby>選択肢<rt>オプション</rt></ruby>| |
...
|<!--`...u8`, `...i32`, `...f64`, `...usize`, etc.-->`...u8`、 `...i32`、 `...f64`、 `...usize`など|<!--Numeric literal of specific type-->特定の型の数値<ruby>直書き<rt>リテラル</rt></ruby>|
|`"..."`|<!--String literal-->文字列<ruby>直書き<rt>リテラル</rt></ruby>|
|<!--`r"..."`, `r#"..."#`, `r##"..."##`, etc.-->`r"..."`、 `r#"..."#`、 `r##"..."##`など。|<!--Raw string literal, escape characters not processed-->生の文字列<ruby>直書き<rt>リテラル</rt></ruby>、エスケープ文字は処理されません|
|`b"..."`|<!--Byte string literal;-->バイト文字列<ruby>直書き<rt>リテラル</rt></ruby>。<!--constructs a `[u8]` instead of a string-->文字列の代わりに`[u8]`を作成する|
|<!--`br"..."`, `br#"..."#`, `br##"..."##`, etc.-->`br"..."`、 `br#"..."#`、 `br##"..."##`など|<!--Raw byte string literal, combination of raw and byte string literal-->生のバイト文字列<ruby>直書き<rt>リテラル</rt></ruby>、生<ruby>文字列<rt>ストリング</rt></ruby>とバイト文字列<ruby>直書き<rt>リテラル</rt></ruby>の組み合わせ|
|`'...'`|<!--Character literal-->文字<ruby>直書き<rt>リテラル</rt></ruby>|
|`b'...'`|<!--ASCII byte literal-->ASCIIバイト<ruby>直書き<rt>リテラル</rt></ruby>|
|`\|...\| expr`|<!--Closure-->閉包|
|`!`|<!--Always empty bottom type for diverging functions-->分岐機能のために常に空のボトム型|
|`_`|<!--“Ignored” pattern binding;-->"無視される"パターン束縛。<!--also used to make integer literals readable-->整数<ruby>直書き<rt>リテラル</rt></ruby>を読みやすくするためにも使われます|
...
|<!--`path::<...>`, `method::<...>`-->`path::<...>`、 `method::<...>`|<!--Specifies parameters to generic type, function, or method in an expression;-->式の総称型、機能、または<ruby>操作法<rt>メソッド</rt></ruby>へのパラメータを指定します。<!--often referred to as turbofish (eg, `"42".parse::<i32>()`)-->しばしばターボフィッシュと呼ばれる（例えば、`"42".parse::<i32>()`）|
...
表B-5に、総称型のパラメータを<ruby>特性<rt>トレイト</rt></ruby>縛りで制約する文脈で表示されるシンボルを示します。
...
|<!--Symbol-->シンボル|<!--Explanation-->説明|
|--------|-------------|
|`//`|<!--Line comment-->行<ruby>注釈<rt>コメント</rt></ruby>|
|`//!`|<!--Inner line doc comment-->内部線の<ruby>注釈<rt>コメント</rt></ruby>|
|`///`|<!--Outer line doc comment-->外部行の開発資料<ruby>注釈<rt>コメント</rt></ruby>|
|`/*...*/`|<!--Block comment-->注釈を<ruby>段落<rt>ブロック</rt></ruby>する|
|`/*!...*/`|<!--Inner block doc comment-->内部<ruby>段落<rt>ブロック</rt></ruby>文書の<ruby>注釈<rt>コメント</rt></ruby>|
|`/**...*/`|<!--Outer block doc comment-->外部<ruby>段落<rt>ブロック</rt></ruby>の開発資料の<ruby>注釈<rt>コメント</rt></ruby>|
...
|<!--Symbol-->シンボル|<!--Explanation-->説明|
|--------|-------------|
|`()`|<!--Empty tuple (aka unit), both literal and type-->直書きと型の両方の空の組（<ruby>別名<rt>エイリアス</rt></ruby>ユニット）|
...
|`(type, ...)`|<!--Tuple type-->組型|
|`expr(expr, ...)`|<!--Function call expression;-->機能呼び出し式。<!--also used to initialize tuple `struct` s and tuple `enum` <ruby>場合値<rt>バリアント</rt></ruby>s-->組`struct`と組`enum`型の初期化にも使用されます|
...
|<!--Context-->文脈|<!--Explanation-->説明|
|---------|-------------|
|`[...]`|<!--Array literal-->配列<ruby>直書き<rt>リテラル</rt></ruby>|
|`[expr; len]`|<!--Array literal containing `len` copies of `expr`-->`expr` `len`コピーを含む配列<ruby>直書き<rt>リテラル</rt></ruby>|
|`[type; len]`|<!--Array type containing `len` instances of `type`-->含む配列型`len`の<ruby>実例<rt>インスタンス</rt></ruby>`type`|
|`expr[expr]`|<!--Collection indexing.-->集まりの索引付け。<!--Overloadable (`Index`, `IndexMut`)-->多重定義可能（`Index`、 `IndexMut`）|
|<!--`expr[..]`, `expr[a..]`, `expr[..b]`, `expr[a..b]`-->`expr[..]`、 `expr[a..]`、 `expr[..b]`、 `expr[a..b]`|<!--Collection indexing pretending to be collection slicing, using `Range`, `RangeFrom`, `RangeTo`, or `RangeFull` as the “index”-->「<ruby>添字<rt>インデックス</rt></ruby>」として`Range`、 `RangeFrom`、 `RangeTo`、または`RangeFull`を使用して、<ruby>集まり<rt>コレクション</rt></ruby>のスライスを行う<ruby>集まり<rt>コレクション</rt></ruby>の<ruby>添字<rt>インデックス</rt></ruby>作成を行います。|
