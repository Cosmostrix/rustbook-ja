...
* `break`: 〈ループ〉 (loop) からの脱出。[Loops (Ending Iteration Early)]を参照。
* `const`: 項目を定数とする。[`const` と `static`]を参照。
* `continue`: 次の〈ループ〉 (loop) の反復を続ける。[Loops (Ending Iteration Early)]を参照。
...
* `for`: 反復子 (iterator) 〈ループ〉 (loop) 、特性 `impl` 構文の一部、高階の寿命構文。[Loops (`for`)]、[Method Syntax]を参照。
* `if`: 条件分岐。[`if`]、[`if let`]を参照。
* `impl`: 継承と特性実装〈ブロック〉. [Method Syntax]を参照。
* `in`: `for`〈ループ〉構文の一部。[Loops (`for`)]を参照。
* `let`: variable binding. [Variable Bindings]を参照。
